...
function ThemeToggle(): ReactElement {
  ...
  return (
    <button
      className={`icon-button bulb-toggle ${switching ? 'is-switching' : ''}`}
      type="button"
      onClick={switchTheme}
    ...
    >
      ...
      <svg
        className={`theme-bulb ${theme === 'light' ? 'is-on' : 'is-off'}`}
        aria-hidden="true"
        viewBox="0 0 24 24"
      >
        <g className="bulb-rays">
          <path d="M12 1v2M4.9 4.9l1.4 1.4M1.8 12h2M20.2 12h2M17.7 6.3l1.4-1.4" />
        </g>
        <path
          className="bulb-glass"
          d="M8.5 15.1A6 6 0 1 1 15.5 15c-.8.6-1.2 1.4-1.3 2.2H9.8c-.1-.8-.5-1.5-1.3-2.1Z"
        />
    ...
    <div className="site-canvas">
      ...
      <header className="site-header">
        <div className="header-inner">
          <Link className="brand" to="/" aria-label="Igor Sobral, Home">
            <span>I</span>
            <strong>Igor Sobral&nbsp;|&nbsp;Software Engineer</strong>
          </Link>
          <nav className="desktop-nav" aria-label="Primary navigation">
            {links.map((link) => (
              <NavLink className="nav-link" key={link.to} to={link.to} end={link.end}>
                {link.label}
              </NavLink>
            ))}
          </nav>
      ...
      <footer className="site-footer">
        <div className="container footer-inner">
          <div>
            <h2>Have a good problem to solve?</h2>
            {data?.profile.email ? (
              <a className="footer-contact" href={`mailto:${data.profile.email}`}>
                {data.profile.email} ↗
              </a>
            ) : (
              <span className="footer-contact">Contact details coming soon</span>
            )}
          </div>
          <div className="footer-meta">
            <p>{data?.profile.location ?? 'Based in Brazil'} · Working worldwide</p>
            {location.pathname !== '/' && <Link to="/">← Back to Home</Link>}
            {/* Blog-aware back link retained for later: <Link to={location.pathname.startsWith('/articles/') ? '/articles' : '/'}>...</Link> */}
          </div>
        </div>
      </footer>
    </div>
